...
function App() {
  const { mode, setMode, setConfig, setShell, config, repoIndexStatus } = useStore();
  const repoLabel = config.owner && config.repo ? `${config.owner}/${config.repo}` : '';
  const repoIndexClassName =
    repoIndexStatus?.status === 'indexed'
      ? 'success'
    ...
    <div className="app">
      ...
      <div className="app-main">
        ...
        <div className="app-status-bar" role="status" aria-live="polite">
          ...
          <div className="status-item">{MODE_LABELS[mode as AppMode] ?? mode}</div>
          {repoLabel && (
            <>
              <div className="status-divider" />
