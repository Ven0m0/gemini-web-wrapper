...
const GitClonePanel: React.FC<GitClonePanelProps> = ({ githubToken, onPrepare }) => {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--color-text-muted, #aaa)' }}>
        Git Clone Helper
      </div>
      ...
      {canUseToken && (
        <label style={{ fontSize: 11, color: 'var(--color-text-muted, #aaa)', display: 'flex', alignItems: 'center', gap: 4 }}>
          <input
            type="checkbox"
            checked={useToken}
      ...
      {error && (
        <div style={{ fontSize: 11, color: 'var(--color-error, #d14d41)' }}>{error}</div>
      )}
    </div>
  )
}
  ...
  return (
    <div className="tool-container" style={{height: '100vh', display: 'flex', flexDirection: 'column'}}>
      ...
      <GitClonePanel
        ...
        onPrepare={(cmd) => setWebShell({ prepared: cmd })}
      />
      ...
      {webShell.prepared && (
        <div style={{padding: '0.5rem', background: '#111', borderTop: '1px solid #333'}}>
          ...
          <pre onClick={handleCopy} style={{whiteSpace: 'pre-wrap', overflow: 'auto', fontSize: 12, cursor: 'pointer'}}>{webShell.prepared}</pre>
        </div>
      )}
    </div>
  )
