...
import { type GitHubDirectoryItem, GitHubService } from '../services/github';
import { RepoIndexService, type RepoIndexStatus, type RepoSearchResult } from '../services/repoIndex';
import { getActiveWebSocketService, setActiveWebSocketService, WebSocketService } from '../services/websocket';
...
export const Tool: React.FC = () => {
  ...
  const loadDirectory = useCallback(
    async (path: string = ROOT_PATH, options?: { silent?: boolean }) => {
  ...
  const handleSearchLoadedFile = () => {
    ...
    try {
      const results = searchAnnotatedContent(file.current, searchPattern, Number.parseInt(searchContext, 10) || 2);
  ...
  const handleIndexRepository = async () => {
    ...
    try {
      ...
      setRepoSearchResults([]);
      addLog(
        `Indexed ${status.indexed_files} file${status.indexed_files === 1 ? '' : 's'} with ${status.symbol_count} symbol${status.symbol_count === 1 ? '' : 's'}`
      );
    } catch (error) {
  ...
  const renderTree = (path: string = ROOT_PATH, depth = 0): React.ReactNode => {
    ...
    return items.map((item) => {
      ...
      return (
        <div key={item.path}>
          <div className={`tree-node ${isSelected ? 'selected' : ''}`} style={{ paddingLeft: `${depth * 14 + 8}px` }}>
            {isDirectory ? (
              <button
                type="button"
            ...
            <div className="workspace-tree">
              {!hasGitHubConfig ? (
                <div className="workspace-empty-state">
                  Add your GitHub token, owner, and repo in Settings to load the tree.
                </div>
            ...
            <div className="workspace-editor-header">
              ...
              <div className="workspace-toolbar">
                ...
                <button
                  ...
                  disabled={!activeFilePath || !isJsonPath(activeFilePath)}
                >
                  Repair JSON
                </button>
                <button
                ...
                <button
                  ...
                  disabled={!activeFilePath || downloading}
                >
                ...
                <button
                  type="button"
                  className="upload-btn"
                  onClick={() => void handleSaveLoadedFile()}
                  disabled={!activeFilePath || !hasUnsavedChanges || saving}
                >
                  {saving ? 'Saving…' : 'Save'}
                </button>
              </div>
            ...
            <div className="workspace-main-grid">
              <div className="workspace-panel">
                ...
                <textarea
                  value={file.current}
                  onChange={(event) =>
                    setFile({ current: event.target.value, dirty: event.target.value !== file.original })
                  }
                      ...
                      repoSearchResults.map((result) => (
                        ...
                        >
                          <div className="workspace-search-result-header">
                            ...
                            <span>
                              {result.kind} · {result.start_line}-{result.end_line}
                            </span>
                          </div>
                          <div className="workspace-search-result-name">
                            {result.name} · score {result.score.toFixed(1)}
                          </div>
                ...
                <div className="workspace-panel">
                  ...
                  <div className="workspace-inline-actions">
                    ...
                    <button
                      type="button"
                      className="download-btn"
                      onClick={handleSearchLoadedFile}
                      disabled={!activeFilePath}
                    >
                      Search
                    </button>
      ...
      {toolMode === 'websocket' && (
        <div className="tool-section">
          ...
          <div className="ws-status">
            Status:{' '}
            <span className={websocket.connected ? 'connected' : 'disconnected'}>
          ...
          <div className="tool-mode-switch">
            <button
              onClick={() => void handleWebSocketConnect()}
          ...
          <div className="upload-section">
            ...
            <button
              onClick={() => void handleWebSocketUpload()}
              disabled={uploading || !websocket.connected}
              className="upload-btn"
            >
