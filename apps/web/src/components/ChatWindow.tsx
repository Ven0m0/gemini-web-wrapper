...
export const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const aiService = new AIService(config.openaiKey || '', config.model || 'gpt-4o-mini', config.temperature || 0.3);

      const response = await aiService.transformFile(input.trim(), 'Please respond to this message conversationally.');
          ...
          >
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-white text-opacity-80 text-xs">{isTyping ? 'Typing...' : 'Online'}</p>
              </div>
            </div>
            <div className="flex items-center space-x-2">
              <button
                ...
                >
                  <div className="flex items-start space-x-2">
                    ...
                    <div className="flex-1">
                      <p style={{ color: message.role === 'user' ? '#ffffff' : themeStyles.text }}>{message.content}</p>
                      ...
                      {showTimestamp && <p className="text-xs opacity-60 mt-1">{formatTime(message.timestamp)}</p>}
                    </div>
                  </div>
                </div>
              </div>
