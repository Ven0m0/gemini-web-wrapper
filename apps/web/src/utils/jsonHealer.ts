...
export class JSONHealer {
  ...
  private static fixUnquotedKeys(input: string): string {
    ...
    return input.replace(regex, (_match, stringMatch, prefix, spaceBefore, key, spaceAfter) => {
      if (stringMatch) {
        return stringMatch;
      }
   ...
   */
  static validateSchema<_T>(data: any, schema: any): { valid: boolean; errors?: string[] } {
    const errors: string[] = [];
